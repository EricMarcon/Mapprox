...
Le code R est donné, ce qui permet une parfaite reproductibilité les exemples traités (ou d'en développer d'autres).
...
Seuls les points "Cas" sont représentés sur la fenêtre ci-dessus. 
...
Considérons la simulation des Cas obtenus par le processus de processus de Matérn et découpons la fenêtre en une grille. 
Un maillage est de l'espace précédemment considéré est obtenu.
...
La position recentrée est présentée sur la carte suivante. 
...
L'absence de Cas dans une cellule est aisément détectable (point unicolore bleu), tout comme la forte présence de Cas dans une cellule (point bicolore mais à dominante rouge).
...
On observe que *M* détecte une agglomération des Cas, ce qui est en accord avec la simulation de ce type de points (les contrôles ayant localisation complètement aléatoire sur la fenêtre). 
...
Sur le graphique ci-dessous, l'intervalle de confiance, simulé à 95%, apparaît en gris et est centré sur la valeur 1.
...
Le package *microbenchmark* proposé par @Mersmann est retenu pour chronométrer le temps d'exécution de la fonction *M*.
...
Les barres rouges sur la figure ci-dessus représentent l'écart-type empirique de la valeur de $M$, calculé à partir des simulations.
