...
Le calcul des distances est extrêmement rapide dans la fonction `Mhat()`: la matrice le fait économiser, mais le traitement complet à partir d'une matrice est finalement plus long sur GitHub (où ce document est produit), probablement parce que la mémoire est limitée.
En revanche, le calcul est 5 fois plus rapide sur un ordinateur  sous Windows avec suffisamment de mémoire.
...
```{r}
# Simulate X
X_csr_list <- replicate(simulations_n, X_csr(par_points_nb), simplify = FALSE)
```
